...
pub async fn set_zhi_tool_config(
    ...
    if !is_valid_popup_timeout(request_timeout_ms) {
        return Err(format!(
            "无效的倒计时时间: {}ms（允许范围 1000ms - 3600000ms）",
            request_timeout_ms
        ));
    }
    ...
    log::info!("cunzhi 工具倒计时已更新为: {}ms", request_timeout_ms);
    Ok(())
}
...
pub async fn set_mcp_tool_enabled(
    ...
    if tool_id != mcp::TOOL_ZHI {
        return Err("该集成工具已移除".to_string());
    }

    {
        ...
        if !enabled {
            return Err("cunzhi 工具是必需的，无法禁用".to_string());
        }
